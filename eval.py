...
parser.add_argument('--savepath', default='/home/SHARE2/ZXY/RFNet/RFNet/results_finetune', type=str)
parser.add_argument('--resume', default='/home/SHARE2/ZXY/RFNet/RFNet/results_finetune/finetune_300.pth', type=str)
parser.add_argument('--pretrain', default=None, type=str)
parser.add_argument('--lr', default=2e-4, type=float)
parser.add_argument('--weight_decay', default=1e-4, type=float)
parser.add_argument('--num_epochs', default=300, type=int)
parser.add_argument('--iter_per_epoch', default=150, type=int)
parser.add_argument('--region_fusion_start_epoch', default=1, type=int)#从第100个epoch开始区域融合
...
#params(iterable)：可⽤于迭代优化的参数或者定义参数组的dicts。
#lr(float, optional)：学习率(默认: 1e-3)
#betas(Tuple[float, float], optional)：⽤于计算梯度的平均和平⽅的系数(默认: (0.9, 0.999))
#eps(float, optional)：为了提⾼数值稳定性⽽添加到分母的⼀个项(默认: 1e-8)
#weight_decay(float, optional)：权重衰减(如L2惩罚)(默认: 0)
    ...
    train_set = Brats_loadall_nii_aug(transforms=args.train_transforms, root=args.datapath, num_cls=num_cls, train_file=train_file)
    ...
    if args.resume is not None:
        ...
        test_score = AverageMeter()#计算并存储平均值和当前值
        with torch.no_grad():
            logging.info('###########test set wi post process###########')
            for i, mask in enumerate(masks[::-1]):#倒序输出
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            ...
            x, target, mask = data[:3]#取data的前三个值 x-->torch.Size([1, 4, 80, 80, 80]) target:[1,4,80,80,80]

            x = x.cuda(non_blocking=True)#将x加载到cuda上面去，建议使用model.to(device)的方式代替
            target = target.cuda(non_blocking=True)#将target加载到cuda上面去，建议使用model.to(device)的方式代替
            mask = mask.cuda(non_blocking=True)#将mask加载到cuda上面去，建议使用model.to(device)的方式代替
            ...
            fuse_loss = fuse_cross_loss + fuse_dice_loss#公式2
        ...
        if (epoch+1) % 50 == 0 or (epoch>=(args.num_epochs-10)):
            file_name = os.path.join(ckpts, 'finetune_{}.pth'.format(epoch+1))
            torch.save({
                'epoch': epoch,
