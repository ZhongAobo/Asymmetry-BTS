"""
adapted from  Detectron config file (https://github.com/facebookresearch/Detectron) #改编自Detectron配置文件
"""
...
def setup(args, log):
    ...
    if not os.path.exists(ldir):
        os.makedirs(ldir)#不存在该文件夹就创建该文件夹

    lfile = args.name + '_' + log + '.txt' if log else args.name + '.txt'#如果log有值就已log起名，否则以args.name起名
    lfile = os.path.join(ldir, lfile)

    logging.basicConfig(level=logging.INFO,
            format='%(asctime)s %(message)s', filename=lfile)#logging的基础设置
