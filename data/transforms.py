...
class Base(object):#所有函数自动执行__init__->sample->tf
...
class RandCrop(CenterCrop):
    def sample(self, *shape):#sample从指定序列中随机获取指定长度的片断并随机排列，结果以列表的形式返回
        size = self.size
...
class RandCrop3D(CenterCrop):
    def sample(self, *shape): # shape : [240,240,155]
        ...
        if not isinstance(self.size,list):
            ...
            size = self.size
        start = [random.randint(0, s-i) for i,s in zip(size,shape)]#slice() 函数实现切片对象，主要用在切片操作函数里的参数传递。
        self.buffer = [slice(None)] + [slice(s, s+k) for s,k in zip(start,size)]#zip() 函数用于将对象中对应的元素打包成元组，然后返回由这些元组组成的对象
        return size
