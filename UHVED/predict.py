...
def softmax_output_dice_class4(output, target):#output:[1,1,112,112,112] target:[1,num_cls=4,112,112,112]
    eps = 1e-8
    #######label1########
    o1 = (output == 1).float()
    t1 = (target == 1).float()
...
def test_softmax(
    ...
    one_tensor = torch.ones(1, 112, 112, 112).float().cuda()
    ...
    for i, data in enumerate(test_loader):
        target = data[1].cuda()##################################[1,H,W,Z]
        x = data[0].cuda()######################################[1,4,H,W,Z]
        names = data[-1]
        ...
        h_cnt = np.int64(np.ceil((H - 112) / (112 * (1 - 0.5))))
        h_idx_list = range(0, h_cnt)
        h_idx_list = [h_idx * np.int64(112 * (1 - 0.5)) for h_idx in h_idx_list]
        h_idx_list.append(H - 112)

        w_cnt = np.int64(np.ceil((W - 112) / (112 * (1 - 0.5))))
        w_idx_list = range(0, w_cnt)
        w_idx_list = [w_idx * np.int64(112 * (1 - 0.5)) for w_idx in w_idx_list]
        w_idx_list.append(W - 112)

        z_cnt = np.int64(np.ceil((Z - 112) / (112 * (1 - 0.5))))
        z_idx_list = range(0, z_cnt)
        z_idx_list = [z_idx * np.int64(112 * (1 - 0.5)) for z_idx in z_idx_list]
        z_idx_list.append(Z - 112)
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    weight1[:, :, h:h+112, w:w+112, z:z+112] += one_tensor
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    x_input = x[:, :, h:h+112, w:w+112, z:z+112]
                    images = dict()
                    images['Flair'] = x_input[:,0:1,:,:,:]
                    images['T1c'] = x_input[:,1:2,:,:,:]
                    images['T1'] = x_input[:,2:3,:,:,:]
                    images['T2'] = x_input[:,3:4,:,:,:]
                    pred_part = model(images, mask.unsqueeze(0), True)
                    pred[:, :, h:h+112, w:w+112, z:z+112] += pred_part
        pred = pred / weight
        b = time.time()
        pred = pred[:, :, :H, :W, :T]
