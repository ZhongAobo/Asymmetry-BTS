...
def softmax_output_dice_class4(output, target):#output:[1,1,80,80,80] target:[1,num_cls=4,80,80,80]
    eps = 1e-8
    #######label1########
    o1 = (output == 1).float()#返回一个全为1.和0.的tensor
    t1 = (target == 1).float()
...
def test_softmax(
    ...
    for i, data in enumerate(test_loader):
        ...
        h_cnt = np.int64(np.ceil((H - 80) / (80 * (1 - 0.5))))
        h_idx_list = range(0, h_cnt)
        h_idx_list = [h_idx * np.int64(80 * (1 - 0.5)) for h_idx in h_idx_list]
        h_idx_list.append(H - 80)

        w_cnt = np.int64(np.ceil((W - 80) / (80 * (1 - 0.5))))
        w_idx_list = range(0, w_cnt)
        w_idx_list = [w_idx * np.int64(80 * (1 - 0.5)) for w_idx in w_idx_list]
        w_idx_list.append(W - 80)

        z_cnt = np.int64(np.ceil((Z - 80) / (80 * (1 - 0.5))))
        z_idx_list = range(0, z_cnt)
        z_idx_list = [z_idx * np.int64(80 * (1 - 0.5)) for z_idx in z_idx_list]
