...
from data.datasets_aug import Brats_loadall_nii_mixup, Brats_loadall_test_nii
...
parser.add_argument('--savepath', default='/home/SHARE2/ZXY/RFNet/RFNet/results_mix', type=str)
...
#weight_decay(float, optional)：权重衰减(如L2惩罚)(默认: 0)
    ...
    train_set = Brats_loadall_nii_mixup(transforms=args.train_transforms, root=args.datapath, num_cls=num_cls, train_file=train_file)
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch+1) % 50 == 0 or (epoch>=(args.num_epochs-10)):
            file_name = os.path.join(ckpts, 'model_{}.pth'.format(epoch+451))
            torch.save({
                'epoch': epoch,
